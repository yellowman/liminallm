...
def chunk(fs_path="reports/turbines.md", *, context_id="ctx_a", index=0, content="x"):
...
class TestInlineTextIsNotAFile:
    ...
    path was given. Mapping that to `kind="file", locator="inline"` would make
    every unrelated inline document in the turn one global source called
    "inline", and a citation would point at whichever arrived first."""

    def test_inline_is_neutral_rather_than_a_fabricated_file(self):
        r = SourceRegistry()
        register_retrieved_chunks(r, [chunk("inline", context_id="ctx_a")])
        source = r.get_source("src_1")
        assert source.kind == "unknown"
        assert source.locator is None
        assert source.origin_id == "context:ctx_a:inline"

    def test_two_contexts_inline_text_does_not_merge(self):
        r = SourceRegistry()
        register_retrieved_chunks(
            r,
            [
                chunk("inline", context_id="ctx_a", content="one"),
                chunk("inline", context_id="ctx_b", content="two"),
            ],
        )
        assert len(r.sources) == 2, "unrelated inline documents are not one source"

    def test_inline_within_one_context_is_one_source(self):
        """The identity is context-scoped, so the same context's inline text
        stays one thing however many chunks it produced."""
        r = SourceRegistry()
        register_retrieved_chunks(
            r,
            [
                chunk("inline", context_id="ctx_a", index=0, content="one"),
                chunk("inline", context_id="ctx_a", index=1, content="two"),
            ],
        )
        assert len(r.sources) == 1
        assert len(r.evidence) == 2
...
class TestWhichScopeFoundWhatIsABindingNotAField:
    ...
    def test_the_same_file_through_two_contexts_is_one_source(self):
        ...
        first = register_retrieved_chunks(
            r, [chunk("manual.md", context_id="ctx_a", index=3, content="same bytes")]
        )
        second = register_retrieved_chunks(
            r, [chunk("manual.md", context_id="ctx_b", index=3, content="same bytes")]
        )
        assert len(r.sources) == 1
...
class TestEveryProductionPathThreadsTheTurnsRegistry:
    ...
    #: The one place a fresh registry is legitimately built mid-tree: the
    #: direct tool endpoint, which has no workflow turn around it to share
    #: one with, and so owns its own exactly as it owns its own invocation.
    MAY_CONSTRUCT = {"_invoke_tool"}
    ...
    def _call_sites(self):
        ...
        being threaded, not a construction.
        """
        import ast
        import pathlib
        ...
        for module in (wf, wfs):
            ...
            for node in ast.walk(ast.parse(path.read_text())):
                ...
                if name in self.MAY_CONSTRUCT:
                    continue
                if not (isinstance(value, ast.Name) and value.id == "source_registry"):
