...
def _create_context(store, name: str = "ctx"):
...
def test_store_round_trips_chunk_fields(store):
    ...
    stored = store.list_chunks(context.id)[0]
...
def test_store_rejects_missing_fs_path(store):
    context = _create_context(store)
...
def test_upsert_context_rejects_missing_owner(store):
    with pytest.raises(ConstraintViolation):
